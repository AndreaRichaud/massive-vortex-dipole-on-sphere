function Run_real_time_dynamics_sphere(N_b, output_folder)

%RUN_REAL_TIME_DYNAMICS_SPHERE Evolve a converged massive vortex dipole in real time.
%
%   RUN_REAL_TIME_DYNAMICS_SPHERE(N_b, output_folder) loads the converged
%   state stored in output_folder, removes the pinning potential, and
%   evolves the two coupled Gross-Pitaevskii equations in real time.
%
%   Inputs
%   ------
%   N_b
...
%   output_folder
%       Case directory containing Vortexon_dipole_on_sphere.mat. Results,
%       frames, diagnostic figures, and logs are written to the same folder.
%
%   Main file written
%   -----------------
...
log_file = fullfile(output_folder, 'log_movie_vortexon.txt');
...
try
    fprintf('============================================================\n');
    fprintf('Run_real_time_dynamics_sphere started\n');
    fprintf('N_b = %d\n', N_b);
    ...
    % Load input MAT
    %--------------------------------------------------------------%
    input_mat_file = fullfile(output_folder, 'Vortexon_dipole_on_sphere.mat');
    ...
    %--------------------------------------------------------------%
    output_mat_file = fullfile(output_folder, 'Real_time_dynamics_on_sphere_with_Ec_Ei.mat');
    ...
    fprintf('Run_real_time_dynamics_sphere completed successfully in %.2f s.\n', toc(t_start));

    diary off;

catch ME
    fprintf('\nERROR in Run_real_time_dynamics_sphere\n');
